...
} 2>&1 | tee /data/status/startup.txt
...
if [[ "${AUTORESEARCH_PREPARE_ON_START:-1}" == "1" ]]; then
  (python3 prepare.py --num-shards "${AUTORESEARCH_NUM_SHARDS:-2}" --download-workers "${AUTORESEARCH_DOWNLOAD_WORKERS:-2}" 2>&1 | tee /data/status/prepare.log)
  rc=${PIPESTATUS[0]}
  echo "prepare exit=$rc" | tee -a /data/status/prepare.log
fi

if [[ "${AUTORESEARCH_TRAIN_ON_START:-0}" == "1" ]]; then
  "$APP_DIR/scripts/run-train-once.sh" 2>&1 | tee /data/status/train.log
  rc=${PIPESTATUS[0]}
  echo "train exit=$rc" | tee -a /data/status/train.log
fi
